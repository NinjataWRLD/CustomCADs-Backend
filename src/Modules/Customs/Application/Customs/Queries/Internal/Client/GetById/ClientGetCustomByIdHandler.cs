...
public sealed class ClientGetCustomByIdHandler(ICustomReads reads, IRequestSender sender)
    : IQueryHandler<ClientGetCustomByIdQuery, ClientGetCustomByIdDto>
{
    public async Task<ClientGetCustomByIdDto> Handle(ClientGetCustomByIdQuery req, CancellationToken ct)
    {
        ...
        return custom.ToClientGetByIdDto(timeZone, designer);
    }
}
