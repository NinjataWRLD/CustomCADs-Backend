...
public sealed class PurchaseOrderWithDeliveryHandler(IOrderReads reads, IUnitOfWork uow, IRequestSender sender, IPaymentService payment, IEventRaiser raiser)
    : ICommandHandler<PurchaseOrderWithDeliveryCommand, string>
{
    public async Task<string> Handle(PurchaseOrderWithDeliveryCommand req, CancellationToken ct)
    {
        ...
        await raiser.RaiseDomainEventAsync(new OrderPurchasedWithDeliveryDomainEvent(
            OrderId: req.OrderId,
