...
public sealed class PurchaseOrderWithDeliveryHandler(IOngoingOrderReads ongoingOrdersReads, IWrites<CompletedOrder> completedOrderWrites, IUnitOfWork uow, IRequestSender sender, IPaymentService payment, IEventRaiser raiser)
    : ICommandHandler<PurchaseOngoingOrderWithDeliveryCommand, string>
{
    public async Task<string> Handle(PurchaseOngoingOrderWithDeliveryCommand req, CancellationToken ct)
    {
        ...
        CompletedOrder completedOrder = CompletedOrder.CreateWithDelivery(
            name: order.Name,
        ...
        await raiser.RaiseDomainEventAsync(new OngoingOrderPurchasedWithDeliveryDomainEvent(
