...
public class OrderPurchasedWithDeliveryDomainEventHandler(ICompletedOrderReads reads, IUnitOfWork uow, IRequestSender sender)
{
    public async Task Handle(OngoingOrderPurchasedWithDeliveryDomainEvent de)
    {
        CompletedOrder order = await reads.SingleByIdAsync(de.OrderId, track: false).ConfigureAwait(false)
            ?? throw CompletedOrderNotFoundException.ById(de.OrderId);
